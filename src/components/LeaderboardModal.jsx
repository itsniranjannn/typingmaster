    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Leaderboard">
      <div className={`w-full max-w-2xl mx-4 max-h-[90vh] space-y-6 overflow-y-auto rounded-2xl border p-6 shadow-2xl ${isDark ? "border-slate-700/50 bg-slate-900" : "border-slate-200 bg-white"}`}>
        ...
        {selectedMode === TYPING_MODES.GOAL && (
          <div className="space-y-2">
            <p className="text-sm text-slate-400">Filter Goal results by variant</p>
            <div className="flex flex-wrap gap-2">
        ...
        {filteredResults.length > 0 ? (
          <div className="space-y-2">
            <div className="grid grid-cols-12 gap-2 border-b border-slate-700/50 px-3 py-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
              ...
              <div className="col-span-3">Mode / Variant</div>
            ...
            {filteredResults.map((result, index) => (
              ...
              >
                <div className="col-span-1 font-bold text-brand-500">
                  {index === 0 ? '🥇' : index === 1 ? '🥈' : index === 2 ? '🥉' : `#${index + 1}`}
                </div>
                <div className="col-span-3 flex items-center gap-2 font-semibold text-lg text-white">
                  <span>{result.wpm}</span>
                  <span className="rounded-full border border-slate-600/70 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-300">
                ...
                <div className="col-span-3 flex items-center gap-2 text-slate-200">
